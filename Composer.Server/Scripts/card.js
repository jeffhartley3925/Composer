...
var enumTimeSignature = { "FourFour_1": 4, "FourFour_2": 11, "FourFour_3": 12, "ThreeFour": 3, "TwoFour": 2, "TwoTwo": 0 };
var enumBar = { "Double": 0, "End": 1, "BeginRepeat": 2, "EndRepeat": 3, "BeginEndRepeat": 4, "Standard": 5, "None": 6 };
var enumClef = { "Treble": 1, "Bass": 0, "Alto": 2, "Tenor": 3 };
var enumAccidental = { "Sharp": 25, "Flat": 26, "Natural": 27 };
var enumKey = { "C": 0, "G": 1, "D": 2, "A": 3, "E": 4, "B": 5, "Gb": 12, "Db": 13, "Ab": 8, "Eb": 9, "Bb": 10, "F": 11, "Fs": 6, "Cs": 7 };
var enumNote = { "Whole": 6, "Half": 7, "Quarter": 8, "Eighth": 9, "Sixteenth": 10, "Thirtysecond": 11, "DottedWhole": 96, "DottedHalf": 97, "DottedQuarter": 98, "DottedEighth": 99, "DottedSixteenth": 100, "DottedThirtysecond": 101 };
var enumRest = { "Whole": 0, "Half": 1, "Quarter": 2, "Eighth": 3, "Sixteenth": 4, "Thirtysecond": 102, "DottedWhole": 90, "DottedHalf": 92, "DottedQuarter": 93, "DottedEighth": 94, "DottedSixteenth": 95, "DottedThirtysecond": 103 };
var enumOrientation = { "Up": 0, "Down": 1 };
...
function renderMeasures() {
    ...
    for (var k = 0; k < measures.length; k++) {
        ...
        if (measure.Sequence == 0) {
            ...
            renderStaff(currentEndX, currentStartX, staffOffset + provenanceHeight + topDeadSpace, staff);
            //end staff lines
        ...
        renderMeasure(currentEndX, currentStartX, measure.Y, measure.Bar_Id);
...
//renderMeasure
//#region
function renderMeasure(endX, startX, startY, barId) {

    if (checkContext()) {

        for (var i = 0; i < 5; i++) {
            var endY = i * lineSpacing + startY;
            drawLine(startX, endY, endX, endY)
        }

        if (barId != enumBar.None) {
            renderBar(endX, endY, barId)
        }
    }
}
//#endregion
...
//renderBar
//#region
function renderBar(endX, endY, barId) {
    ctx.lineWidth = defaultLineWidth;
    switch (barId) {
        case enumBar.Standard:
            drawBarLine(endX - 1, endY);
            break;
        case enumBar.Double:
            drawBarLine(endX - 1, endY);
            drawBarLine(endX + 2, endY);
            break;
        case enumBar.End:
            drawBarLine(endX - 6, endY);
            drawBarLine(endX - 2, endY, 3);
            break;
...
function renderStaff(endX, startX, staffOffset, staff) {
    renderStaffLines(endX, startX, staffOffset);
    if (staff != null) {
...
function renderStaffLines(endX, startX, staffOffset) {
    renderMeasure(endX, startX, staffOffset, enumBar.None);
}
...
function drawLine(x1, y1, x2, y2, lineWidth, opacity) {
