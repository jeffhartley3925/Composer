...
namespace Composer.Modules.Composition.ViewModels.UI
{
	public class HyperlinkBarViewModel : BaseViewModel, IHyperlinkBarViewModel, IEventCatcher
	{
		...
		public void OnAddStaff(object obj)
		{
			...
			for (var sFiX = 0; sFiX < sFDensity; sFiX++)
			{
				var sF = StaffManager.Create(sG.Id, sFiX * Defaults.SequenceIncrement);
				...
				sF.Clef_Id = CompositionManager.ClefIds[sFiX];
				Cache.AddStaff(sF);
		...
		private void UpdateMeasurreGroupsAndSequences()
		{
			//var sQ = SequenceManager.GetSequence(mE.Sequence);
			//sQ.Measures.Add(mE);
			//if (sFiX == 0)
			//{
			//	var mG = MeasuregroupManager.Add(sG, mE);
			//	sQ.Measuregroups.Add(mG);
			//}
